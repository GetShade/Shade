//
//  SaturateEffectMetalRenderer.swift
//  Shade
//
...
class GaussianBlurEffectMetalRenderer: NSObject,MTKViewDelegate {
    ...
    func setupPipeline() {
        ...
        let vertexFunction = library.makeFunction(name: "gaussianBlurVertexShader")
...
struct GaussianBlurEffectMetalView: NSViewRepresentable {
    var image: NSImage
    var radius: Float = 0.5
    ...
    class Coordinator: NSObject {
        ...
        var parent: GaussianBlurEffectMetalView
        var renderer: GaussianBlurEffectMetalRenderer
        
        init(_ parent: GaussianBlurEffectMetalView) {
            self.parent = parent
            let device = MTLCreateSystemDefaultDevice()!
            self.renderer =  GaussianBlurEffectMetalRenderer(device: device, image: parent.image)
        }
    }
}

struct GaussianBlurEffectView: View {
    var image = NSImage(resource: .person)
    @State private var blurCenter: CGPoint = .zero
    @State private var blurRadius: Float = 0.5
    ...
    var body: some View {
        VStack {
            GaussianBlurEffectMetalView(image: image, radius: blurRadius)
                .frame(width: 300, height: 300)
            
            Slider(value: $blurRadius, in: 0...1)
                .padding()
        }
    }
}

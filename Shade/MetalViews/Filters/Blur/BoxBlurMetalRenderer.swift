...
class BoxBlurMetalRenderer: NSObject,MTKViewDelegate {
    ...
    func setupPipeline() {
        ...
        let vertexFunction = library.makeFunction(name: "boxBlurVertexShader")
        let fragmentFunction = library.makeFunction(name: "boxBlurFragment")
...
struct BoxBlurMetalView: NSViewRepresentable {
    var image: NSImage
    var radius: Float = 0.5
    ...
    class Coordinator: NSObject {
        ...
        var parent: BoxBlurMetalView
        var renderer: BoxBlurMetalRenderer
        
        init(_ parent: BoxBlurMetalView) {
            self.parent = parent
            let device = MTLCreateSystemDefaultDevice()!
            self.renderer =  BoxBlurMetalRenderer(device: device, image: parent.image)
        }
    }
}
...
struct BoxBlurView: View {
    ...
    var body: some View {
        VStack {
            BoxBlurMetalView(image: image, radius: blurRadius)

...
class BokehBlurMetalRenderer: NSObject,MTKViewDelegate {
    ...
    func setupPipeline() {
        ...
        let vertexFunction = library.makeFunction(name: "bokehBlurVertexShader")
        let fragmentFunction = library.makeFunction(name: "bokehBlurFragment")
    ...
    func draw(in view: MTKView) {
        ...
        renderEncoder.setFragmentBytes(&ringAmount, length: MemoryLayout<Float>.size, index: 1)
        renderEncoder.setFragmentBytes(&ringSize, length: MemoryLayout<Float>.size, index: 2)
...
struct BokehBlurMetalView: NSViewRepresentable {
    var image: NSImage
    var radius: Float
    ...
    func updateNSView(_ uiView: MTKView, context: Context) {
        ...
        context.coordinator.renderer.ringAmount = ringAmount
        context.coordinator.renderer.ringSize = ringSize
    ...
    class Coordinator: NSObject {
        ...
        var parent: BokehBlurMetalView
        var renderer: BokehBlurMetalRenderer
        
        init(_ parent: BokehBlurMetalView) {
            self.parent = parent
            let device = MTLCreateSystemDefaultDevice()!
            self.renderer =  BokehBlurMetalRenderer(device: device, image: parent.image)
        }
    }
}
...
struct BokehBlurView: View {
      ...
      @State private var ringSize: Float = 0.1
      @State private var ringAmount: Float = 1.0
     var image = NSImage(resource: .person)
      
      var body: some View {
          VStack {
              BokehBlurMetalView(image: image,radius: radius, ringAmount: ringAmount, ringSize: ringSize)
                  .frame(width: 300, height: 300)
                  .border(Color.black)
              
              VStack {
                  Text("Radius: \(radius, specifier: "%.2f")")
                  Slider(value: $radius, in: 0...1) {
                      Text("Radius")
                  }

                  Text("Ring Size: \(ringSize, specifier: "%.2f")")
                  Slider(value: $ringSize, in: 0...1) {
                      Text("Ring Size")
                  }

                  Text("Ring Amount: \(ringAmount, specifier: "%.2f")")
                  Slider(value: $ringAmount, in: 1...10) {
                      Text("Ring Amount")
                  }
              }
              .padding()

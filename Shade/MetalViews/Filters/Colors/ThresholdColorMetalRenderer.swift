//
//  ThresholdColorMetalRenderer.swift
//  Shade
//
...
class ThresholdColorMetalRenderer: NSObject,MTKViewDelegate {
    ...
    func setupPipeline() {
        ...
        let vertexFunction = library.makeFunction(name: "thresholdColorVertexShader")
        let fragmentFunction = library.makeFunction(name: "thresholdColorFragment")
    ...
    func draw(in view: MTKView) {
        ...
        renderEncoder.setFragmentBytes(&threshold, length: MemoryLayout<Float>.size, index: 0)
...
struct ThresholdColorMetalView: NSViewRepresentable {
    var image: NSImage
    var threshold: Float
    ...
    func updateNSView(_ uiView: MTKView, context: Context) {
        context.coordinator.renderer.threshold = threshold
    ...
    class Coordinator: NSObject {
        ...
        var parent: ThresholdColorMetalView
        var renderer: ThresholdColorMetalRenderer
        
        init(_ parent: ThresholdColorMetalView) {
            self.parent = parent
            let device = MTLCreateSystemDefaultDevice()!
            self.renderer =  ThresholdColorMetalRenderer(device: device, image: parent.image)
        }
    }
}
...
struct ThresholdColorView: View {
    @State private var threshold: Float = 0.5 // Default threshold value
    var image = NSImage(resource: .person)
    var body: some View {
        VStack {
            ThresholdColorMetalView(image:image, threshold:threshold)

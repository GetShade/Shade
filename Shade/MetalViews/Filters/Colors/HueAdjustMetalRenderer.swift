...
class HueAdjustMetalRenderer: NSObject,MTKViewDelegate {
    ...
    func setupPipeline() {
        ...
        let vertexFunction = library.makeFunction(name: "hueAdjustVertexShader")
        let fragmentFunction = library.makeFunction(name: "hueAdjustFragment")
    ...
    func draw(in view: MTKView) {
        ...
        renderEncoder.setFragmentBytes(&hueAngle, length: MemoryLayout<Float>.size, index: 0)
...
struct HueAdjustMetalView: NSViewRepresentable {
    var image: NSImage
    var hueAngle: Float
    ...
    func updateNSView(_ uiView: MTKView, context: Context) {
        context.coordinator.renderer.hueAngle = hueAngle
    ...
    class Coordinator: NSObject {
        ...
        var parent: HueAdjustMetalView
        var renderer: HueAdjustMetalRenderer
        
        init(_ parent: HueAdjustMetalView) {
            self.parent = parent
            let device = MTLCreateSystemDefaultDevice()!
            self.renderer =  HueAdjustMetalRenderer(device: device, image: parent.image)
        }
    }
}
...
struct HueAdjustView: View {
    @State private var hueAngle: Float = 0.0 // Angle for hue adjustment
    var image = NSImage(resource: .person)
    var body: some View {
        VStack {
            HueAdjustMetalView(image:image, hueAngle:hueAngle)
                .frame(width: 300, height: 300)

            
            Slider(value: $hueAngle, in: -180...180, step: 1.0, label: {
                Text("Hue Angle: \(Int(hueAngle))°")
            })
        }
    }
}

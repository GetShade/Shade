//
//  ExposureAdjustMetalRenderer.swift
//  Shade
//
...
class ExposureAdjustMetalRenderer: NSObject,MTKViewDelegate {
    ...
    func setupPipeline() {
        ...
        let vertexFunction = library.makeFunction(name: "exposureAdjustVertexShader")
        let fragmentFunction = library.makeFunction(name: "exposureAdjustFragment")
    ...
    func draw(in view: MTKView) {
        ...
        renderEncoder.setFragmentBytes(&exposureEV, length: MemoryLayout<Float>.size, index: 0)
...
struct ExposureAdjustMetalView: NSViewRepresentable {
    var image: NSImage
    var exposureEV: Float = 0.0
    ...
    func updateNSView(_ uiView: MTKView, context: Context) {
        context.coordinator.renderer.exposureEV = exposureEV
    ...
    class Coordinator: NSObject {
        ...
        var parent: ExposureAdjustMetalView
        var renderer: ExposureAdjustMetalRenderer
        
        init(_ parent: ExposureAdjustMetalView) {
            self.parent = parent
            let device = MTLCreateSystemDefaultDevice()!
            self.renderer =  ExposureAdjustMetalRenderer(device: device, image: parent.image)
        }
    }
}
...
struct ExposureAdjustView: View {
    @State private var exposureEV: Float = 0.0 // Exposure Value in percentage
    var image = NSImage(resource: .person)
    var body: some View {
        VStack {
            ExposureAdjustMetalView(image:image,exposureEV: exposureEV)
                .frame(width: 300, height: 300)

            Slider(value: $exposureEV, in: -100...100, label: {
                Text("Exposure EV")
            })
        }
    }
